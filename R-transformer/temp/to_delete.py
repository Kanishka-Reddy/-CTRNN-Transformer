...
#             h_t = torch.matmul(self.W, concat.T) + self.b.unsqueeze(-1)
#             h_t = torch.tanh(h_t).T
#
#         if self.batch_first:
...
class LocalRNN(nn.Module):
    def __init__(self, input_dim, output_dim, rnn_type, ksize, dropout):
        ...
        if rnn_type == 'GRU':
            ...
            self.rnn = CF_CT_RNN(output_dim, output_dim, batch_first=True)
        elif rnn_type == 'LSTM':
